...
export class PhoneValidationService {
  ...
  //     // if (excludeEmptyEmail) {
  //     //   const usersWithEmail = conflictingUsers.filter(user => user.email && user.email.trim() !== '');
        
  //     //   if (usersWithEmail.length === 0) {
  //     //     return {
  //     //       isDuplicate: true,
  //     //       conflictUser: conflictingUsers[0],
  //     //       message: 'Phone number already exists but user has no email',
  //     //     };
  //     //   }
  //     // }
  ...
  //       if (duplicateCheck.isDuplicate && !updatePhone) {
  //         return {
  //           isValid: false,
  //           message: 'phone already mapped to other account',
  //         };
  //       }
  ...
  //       if (currentUser.phone !== internationalFormat) {
  //         updateData.phone_verified = null;
  //       }

  //       // Set phone verification if requested
  //       if (phoneVerified && !currentUser.phone_verified) {
  //         updateData.phone_verified = new Date();
  //         updateData.verified = 1;
  //       }

  //       // Handle duplicate phone cleanup if updating
  //       if (duplicateCheck.isDuplicate && updatePhone && currentUser.phone_verified) {
  //         // Clear u_phone from other users with same number
  ...
  //       // Set verified phone if user is verified
...
// Extract the main logic into a separate method
  ...
  // ): Promise<PhoneUpsertResult> {
  //   // **MOVE DUPLICATE CHECK OUTSIDE TRANSACTION OPERATIONS**
  //   // Check for duplicates using normalized phone BEFORE any database operations
  //   const duplicateCheck = await this.checkPhoneDuplicate(phone, userId);
    
  //   if (duplicateCheck.isDuplicate) {
  //     // **THROW ERROR IMMEDIATELY - DON'T PROCEED WITH UPDATE**
  //     throw new Error('Phone number already mapped to other account');
  //   }
  ...
  //   // Reset phone verification if phone changed
  //   const updateData: any = {
  //     phone: internationalFormat, // Store in international format
  //     u_phone: null, // Reset verified phone initially
  //     modified: new Date(),
  //   };
  ...
  //   // Set phone verification if requested
  //   if (phoneVerified && (!currentUser.phone_verified || phoneChanged)) {
  //     updateData.phone_verified = new Date();
  //     updateData.verified = true;
  //   }
